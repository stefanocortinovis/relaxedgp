...
def sample_discrete(unnormalized_probs):
    ...
    Z = np.sum(np.clip(unnormalized_probs, 0, None))
    if Z == 0:  # if all of the probabilities are numerically 0, sample uniformly
        warnings.warn("Trying to sample discrete distribution with all 0 weights")
        return np.random.choice(a=N, size=1)[0]
    return np.random.choice(a=N, size=1, p=unnormalized_probs / Z)[0]


class InducingPointInitializer:
    def __init__(
        self, seed: Optional[int] = 0, randomized: Optional[bool] = True, **kwargs
    ):
...
class UniformSubsample(InducingPointInitializer):
    ...
    def compute_initialisation(
        ...
        """
        Returns a subset of inducing inputs Z ⊂ 𝑋, with |Z| = M such that the
            probability of any such set is equal.
        :param training_inputs: An array of training inputs X ⊂ 𝑋, with |X| = N < ∞.
            We frequently assume X= ℝ^D and this is [N,D]
        ...
        indices = np.random.choice(
            N, size=M, replace=False
        )  # we always sample without replacement
...
class Kmeans(InducingPointInitializer):
    ...
    def compute_initialisation(
        ...
        :param training_inputs:  An array of training inputs X ⊂ 𝑋, with |X| = N < ∞.
            We frequently assume X= ℝ^D
        and this is [N,D]
        :param M: integer, number of inducing points to return. Equiv. "k" to use in
            kmeans
        :param kernel: unused argument
        :return: Z, None, M inducing inputs
        """
        # data us assumed to be already normalized

        N = training_inputs.shape[0]

        if self.max_data is not None and N > self.max_data:
            uniform = UniformSubsample(0)
            training_inputs, _ = uniform(training_inputs, self.max_data, kernel)

        centroids, _ = scipy.cluster.vq.kmeans(training_inputs, M)
        # Some times K-Means returns fewer than K centroids, in this case we sample
        # remaining point from data
        if len(centroids) < M:
...
class ConditionalVariance(InducingPointInitializer):
    def __init__(
        ...
        **kwargs,
        ...
        """
        :param sample: bool, if True, sample points into subset to use with weights
            based on variance, if False choose
        point with highest variance at each iteration
        :param threshold: float or None, if not None, if tr(Kff-Qff)<threshold, stop
            choosing inducing points as the approx.
        has converged.
        """
        super().__init__(seed=seed, randomized=True, **kwargs)
        self.sample = sample
    ...
    def compute_initialisation(
        ...
        """
        The version of this code without sampling follows the Greedy approximation to
            MAP for DPPs in
        ...
        It is equivalent to running a partial pivoted Cholesky decomposition on Kff
            (see Figure 2 in the below ref.),
        ...
        TODO: IF M ==1 this throws errors, currently throws an assertion error, but
            should fix
        Initializes based on variance of noiseless GP fit on inducing points currently
            in active set
        Complexity: O(NM) memory, O(NM^2) time
        :param training_inputs: [N,D] numpy array,
        :param M: int, number of points desired. If threshold is None actual number
            returned may be less than M
        :param kernel: kernelwrapper object
        :return: inducing inputs, indices,
        [M,D] np.array to use as inducing inputs,  [M], np.array of ints indices of
            these inputs in training data array
        """
        N = training_inputs.shape[0]
        ...
        for m in range(M - 1):
            ...
            if np.sum(np.clip(di, 0, None)) < self.threshold:
                indices = indices[:m]
                warnings.warn(
                    "ConditionalVariance: Terminating selection of "
                    "inducing points early."
                )
                break
        indices = indices.astype(int)
        Z = training_inputs[indices]
